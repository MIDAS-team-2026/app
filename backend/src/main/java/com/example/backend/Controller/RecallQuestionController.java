...
public class RecallQuestionController {
    ...
    // 사용자별 회상 질문 조회 (파이썬 또는 클라이언트에서 호출할 GET 엔드포인트)
    @GetMapping("/questions/{userId}")
    public ResponseEntity<List<RecallQuestionResponseDTO>> getRecallQuestions(
            @PathVariable Integer userId) {

        List<RecallQuestionResponseDTO> response =
                recallQuestionService.getQuestionsByUserId(userId);
    ...
    public ResponseEntity<RecallQuestionResponseDTO> createQuestion(
            @RequestBody RecallQuestionCreateDTO dto) {

        RecallQuestionResponseDTO response =
                recallQuestionService.createQuestion(dto);
    ...
    public ResponseEntity<String> updateQuestion(
            ...
            @RequestBody RecallQuestionUpdateDTO dto) {
