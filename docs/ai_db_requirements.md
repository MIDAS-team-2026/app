...
1. 사용자가 앱에서 자유대화를 시작합니다.
2. 백엔드는 `chat_sessions`에 점검 세션 1건을 생성합니다.
3. 사용자가 질문에 답변할 때마다 음성 파일을 서버 또는 저장소에 저장합니다.
4. 데이터베이스에는 음성 파일 자체가 아니라 파일 경로와 기본 정보를  'audio_records'에 저장합니다.
5. STT 기능으로 음성을 텍스트로 변환하고, 변환 결과를 `audio_records.transcript_text`에 저장합니다.
6. AI 모듈이 각 `audio_records`의 음성 파일에서 음성 특징을 추출합니다.
7. 변환된 텍스트를 기반으로 텍스트 특징을 분석합니다.
8. 회상 질문의 경우, 초기 답변과 현재 회상 답변은 모두 `audio_records`에 저장하고, `parent_record_id` 또는 `recall_analysis_results`로 비교 관계를 연결합니다.
9. AI 모듈은 의미 유사도와 핵심 키워드 일치도를 기반으로 회상 일치도 점수를 계산합니다.
10. 세션 안의 음성 분석 결과, 텍스트 분석 결과, 회상 일치도 결과를 종합하여 최종 위험도 점수를 계산합니다.
11. 백엔드는 분석 결과를 `risk_analysis_results`에 저장하고, 앱은 해당 결과를 조회하여 화면에 표시합니다.
...
### chat_sessions
사용자의 점검/자유대화 1회 전체를 저장하는 테이블입니다.

### audio_records
세션 안에서 발생한 사용자 발화/녹음 1건과 STT 결과를 저장하는 테이블입니다.
...
- 타입: DATETIME
- Null 허용: No
- 설명: 사용자 정보가 생성된 시각입니다.

---

## 5. chat_sessions 테이블
사용자의 자유대화 1회 전체를 저장하는 테이블입니다.
...
- 타입: BIGINT 또는 UUID
- Null 허용: No
- 설명: 해당 녹음이 포함된 자유대화 세션 ID입니다.
- 연결: chat_sessions 테이블의 session_id와 연결됩니다.
...
- 타입: INT
- Null 허용: No
- 설명: 세션 안에서 해당 발화가 몇 번째 순서인지 나타냅니다.

### question_type
- 타입: VARCHAR
- Null 허용: Yes
- 설명: 질문 또는 답변 유형입니다.
- 예시: GENERAL, FACT, PREFERENCE, MEMORY, DAILY, RECALL

### recall_question_id
- 타입: BIGINT 또는 UUID
- Null 허용: Yes
- 설명: 해당 발화가 회상 질문과 관련된 경우 연결되는 회상 질문 ID입니다.
- 연결: recall_questions 테이블의 recall_question_id와 연결됩니다.

### parent_record_id
- 타입: BIGINT 또는 UUID
- Null 허용: Yes
- 설명: 회상 답변이 비교해야 하는 과거 기준 발화의 record_id입니다.
- 비고: 일반 발화 또는 회상과 무관한 발화는 null로 둘 수 있습니다.

### answer_role
- 타입: VARCHAR
- Null 허용: Yes
- 설명: 회상 비교에서 답변의 역할입니다.
- 예시: INITIAL, RECALL
...
- 타입: BIGINT 또는 UUID
- Null 허용: No
- 설명: 위험도 분석 대상이 되는 자유대화 세션 ID입니다.
- 연결: chat_sessions 테이블의 session_id와 연결됩니다.
...
## 10. 테이블 관계
### users와 chat_sessions
- users 1 : N chat_sessions
### chat_sessions와 audio_records
- chat_sessions 1 : N audio_records
### audio_records와 speech_analysis_results
- 1 : 1
### audio_records와 text_analysis_results
- 1 : 1 
### chat_sessions와 risk_analysis_results
- 1 : 1
### users와 recall_questions
- users 1 : N recall_questions
### recall_questions와 recall_keywords
- 1 : N
### recall_questions와 recall_analysis_results
- 1 : N
### audio_records와 recall_analysis_results
- 1 : N
- 비고: 초기 답변 record_id와 현재 회상 답변 record_id를 비교합니다.
...
## 14. recall_analysis_results 테이블
...
- 타입: BIGINT 또는 UUID
- Null 허용: No
- 설명: 비교 기준이 되는 초기 답변의 녹음 기록 ID입니다.
- 연결: audio_records 테이블의 record_id와 연결됩니다.

### current_record_id
- 타입: BIGINT 또는 UUID
- Null 허용: No
- 설명: 현재 회상 답변의 녹음 기록 ID입니다.
- 연결: audio_records 테이블의 record_id와 연결됩니다.
...
## 16. 치매 및 구음장애 데이터 기반 위험도 모델 확장 방향
